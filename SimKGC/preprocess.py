...
parser.add_argument('--test-path', default='', type=str, metavar='N',
                    help='path to valid data')
parser.add_argument('--use-mm', action='store_true', default=False,
                    help='use multimodal data (description, context)')
parser.add_argument('--desc-path', default='entityid2mm_description.del',
                    help='path to multimodal description')
parser.add_argument('--mm-concate', action='store_true', default=False,
                    help='concate mm description + gt description (first sentence of each part)')
...
def _load_mkg_id2rel(path: str):
    ...
    for line in open(path, 'r', encoding='utf-8'):
        ...
        if args.task.lower() == 'fb15k237':
            rel_text = _normalize_fb15k237_relation(rel_text)
        mkg_id2rel[rel_id] = rel_text

    print('Load {} relations from {}'.format(len(mkg_id2rel), path))


def _load_mkg_wikidata(path: str):
    global mkg_id2ent, mkg_id2desc
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        assert len(fs) == 2, 'Invalid line: {}'.format(line.strip())
        entity_id, name = fs[0], fs[1]
        name = name.replace('_', ' ').strip()
        if entity_id not in mkg_id2desc:
            print('No desc found for {}'.format(entity_id))
        mkg_id2ent[entity_id] = (entity_id, name, mkg_id2desc.get(entity_id, ''))
    print('Load {} entity names from {}'.format(len(mkg_id2ent), path))


def _load_mkg_desc(path: str):
    global mkg_id2desc
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        assert len(fs) == 2, 'Invalid line: {}'.format(line.strip())
        entity_id, desc = fs[0], fs[1]
        mkg_id2desc[entity_id] = _truncate(desc, 50)
    print('Load {} entity descriptions from {}'.format(len(mkg_id2desc), path))


def _add_mkg_desc(path: str):
    global mkg_id2desc
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        assert len(fs) == 2, 'Invalid line: {}'.format(line.strip())
        entity_id, desc = fs[0], fs[1]
        # concate mm description + gt description (first sentence of each part)
        mkg_id2desc[entity_id] = f"{'. '.join(mkg_id2desc[entity_id].split('. ')[:2])}. {'. '.join(_truncate(desc, 50).split('. ')[:2])}.".replace('..', '.')
    print('Load {} entity descriptions from {}'.format(len(mkg_id2desc), path))


def _process_line_mkg(line: str) -> dict:
    fs = line.strip().split('\t')
    assert len(fs) == 3, 'Expect 3 fields for {}'.format(line)
    head_id, relation, tail_id = fs[0], fs[1], fs[2]

    _, head, _ = mkg_id2ent[head_id]
    _, tail, _ = mkg_id2ent[tail_id]
    example = {'head_id': head_id,
               'head': head,
               'relation': relation,
               'relation_id': relation,
...
def preprocess_mkg(path):
    if not mkg_id2desc:
        if args.use_mm:
            print('''Use multimodal data, load entity description from {}'''.format(args.desc_path))
            if args.mm_concate:
                _load_mkg_desc('{}/entity_desc.del'.format(os.path.dirname(path)))
                print('Concate mm description with ground truth description')
                _add_mkg_desc('{}/{}'.format(os.path.dirname(path), args.desc_path))
            else:
                _load_mkg_desc('{}/{}'.format(os.path.dirname(path), args.desc_path))

        else:
            _load_mkg_desc('{}/entity_desc.del'.format(os.path.dirname(path)))
    if not mkg_id2ent:
        _load_mkg_wikidata('{}/entity_mentions.del'.format(os.path.dirname(path)))
    if not mkg_id2rel:
        _load_mkg_id2rel('{}/relation_mentions.del'.format(os.path.dirname(path)))

    lines = open(path, 'r', encoding='utf-8').readlines()
    pool = Pool(processes=args.workers)
    examples = pool.map(_process_line_mkg, lines)
    pool.close()
    pool.join()

    _normalize_relations(examples, normalize_fn=lambda rel_id: mkg_id2rel.get(rel_id, None), is_train=(path == args.train_path))
...
def main():
    ...
    for path in [args.train_path, args.valid_path, args.test_path]:
        ...
        elif args.task.lower() == 'fb15k237-ori':
            all_examples += preprocess_fb15k237(path)
        elif args.task.lower() in ['mkg_w', 'mkg_y', 'fb15k237']:
            all_examples += preprocess_mkg(path)
    ...
    if args.task.lower() == 'wn18rr':
        ...
        id2text = {k: v[2] for k, v in mkg_id2ent.items()}
    elif args.task.lower() == 'mkg_w':
        id2text = {k: v[2] for k, v in mkg_id2ent.items()}
    elif args.task.lower() == 'mkg_y':
        id2text = {k: v[2] for k, v in mkg_id2ent.items()}
    elif args.task.lower() in ['wiki5m_trans', 'wiki5m_ind']:
