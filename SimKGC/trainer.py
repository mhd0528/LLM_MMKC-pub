...
from dict_hub import build_tokenizer, get_train_triplet_dict, TripletDict
from logger_config import logger

train_triplet_dict: TripletDict = get_train_triplet_dict()
...
class Trainer:
    ...
    def __init__(self, args, ngpus_per_node):
        ...
        # load mm context if provided
        dataset_path = os.path.dirname(args.train_path)
        args.relation_cnt = len(train_triplet_dict.relations) // 2
        if args.use_mm:
            logger.info('Use multimodal context')
            # triples grouped by relation and tail entity type
            with open(os.path.join(dataset_path, 'context', args.mm_rel_triples), 'r') as f:
                args.mm_rel_triples = json.load(f)
            # possible tail entity types for given head entity and relation
            with open(os.path.join(dataset_path, 'context', args.mm_triple_tail_types), 'r') as f:
                args.mm_triple_tail_types = json.load(f)
            # relation templates as description
            args.rel_descs = read_file(os.path.join(dataset_path, 'relationid2mm_description.del'), 'desc')
            if args.use_relation_hints:
                with open(os.path.join(dataset_path, 'context', args.mm_relation_hints), 'r') as f:
                    args.rel_hints = json.load(f)
        if args.use_reverse_alias:
            logger.info('Use alias for reverse relation')
            args.reverse_alias = read_file(os.path.join(dataset_path, 'reverse_relation_mentions.del'), 'alias')
