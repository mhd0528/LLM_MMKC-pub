...
class BaseDataset(Dataset):
    def __init__(self, configs, tok, triples, text_dict, mode='train', gt=None, lars_dict=None):
        ...
        if self.configs.use_mm:
            self.rel_descs = text_dict['rel_descs']
            self.rel_triples = text_dict['rel_triples']
            self.tail_types = text_dict['tail_types']
            if self.configs.use_relation_hints:
                self.rel_hints = text_dict['rel_hints']
    ...
    def construct_input_text(self, src_ent=None, rel=None, timestamp=None, predict='predict_tail'):
        ...
        # if mm context is available, we want to provide additional context for relation as well
        if self.configs.use_mm:
            forward_rel = rel
            if predict == 'predict_head':  # reverse relation uses rel + rel_cnt as id
                rel = rel + self.configs.n_rel
                if self.configs.use_reverse_alias:
                    rel_name = self.rel_names[rel]
            # get the relation triples based on possible types
            if self.configs.use_relation_triples:
                # first get tail types based on the head and relation
                tail_types = self.tail_types[f"{src_ent}_{rel}"]['tail_types']
                rel_desc = self.rel_descs[rel]
                select_rel_triples = []
                for t_type in tail_types:
                    type_triples = random.sample(list(self.rel_triples[str(rel)][t_type]), 1)
                    select_rel_triples.extend(type_triples)
                rel_name += f" :{rel_desc} {'|SEP|'.join([f'{h}|{r}|{t}' for h, r, t in select_rel_triples])}"
            elif self.configs.use_relation_hints:  # TODO: add relation hints
                # get relation hints based on src_ent and rel
                # relation id is always the forward relation id
                rel_hints = self.rel_hints[str(src_ent)].get(str(forward_rel), '')
                rel_name += f" :{rel_hints}"
        src_desc = ':' + self.ent_descs[src_ent] if self.configs.desc_max_length > 0 else ''
        # print(src_name, src_desc, rel_name, timestamp)
        # print(rel, self.rel_names)
