...
def main():
    ...
    if configs.use_mm:
        ent_descs = read_file(configs, configs.dataset_path, configs.dataset, configs.desc_path, 'desc')
        # ent_descs = read_file(configs, configs.dataset_path, configs.dataset, 'entityid2description.txt', 'desc')
        rel_descs = read_file(configs, configs.dataset_path, configs.dataset, 'relationid2mm_description.txt', 'desc')
        with open(os.path.join(configs.dataset_path, configs.dataset, 'context', configs.mm_rel_triples), 'r') as f:
            rel_triples = json.load(f)
        with open(os.path.join(configs.dataset_path, configs.dataset, 'context', configs.mm_triple_tail_types), 'r') as f:
            tail_types = json.load(f)
        if configs.use_relation_hints:
            with open(os.path.join(configs.dataset_path, configs.dataset, 'context', configs.mm_relation_hints), 'r') as f:
                rel_hints = json.load(f)
    else:
        ent_descs = read_file(configs, configs.dataset_path, configs.dataset, configs.desc_path, 'desc')
    print(f"entity description check: {ent_descs[0]}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-use_mm', action='store_true', help='Use multimodal context')
    parser.add_argument('-desc_path', default='entityid2mm_description.txt', type=str, help='Entity description file')
    parser.add_argument('-use_reverse_alias', action='store_true', help='Use alias for reverse relation')
    parser.add_argument('-use_relation_triples', action='store_true', help='Use triples as relation context')
    parser.add_argument('-use_relation_hints', action='store_true', help='Use generated hints as relation context')
    parser.add_argument('-mm_rel_triples', default='qwen-MKG-W-rel_context-ent_v6-v2.json', type=str, help='multimodal relation triples for tail types')
    parser.add_argument('-mm_triple_tail_types', default='qwen-MKG-W-triple_tail-ent_v6-v1.json', type=str, help='possible tail types for given head and relation')
    parser.add_argument('-mm_relation_hints', default='qwen-MKG-Y+_img_context-setting_3+-combine.json', type=str, help='conceptual hints for entity+relation pairs')
